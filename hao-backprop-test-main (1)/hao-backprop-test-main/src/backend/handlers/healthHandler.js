 ...
 * 
 * This module handles requests to the /health endpoint for monitoring purposes,
 * validates HTTP methods, and generates appropriate responses with health status
 ...
 * and generating appropriate responses with health status in JSON format
...
function handleHealthRequest(req, res) {
  ...
  if (isGetMethod(method)) {
    ...
    // Send health status JSON response
    res.end(JSON.stringify({ status: MESSAGES.HEALTH_OK }));
    
    // Log the successful response
    logger.info(`Successfully responded with ${HTTP_STATUS.OK} OK and health status`);
  } else {
