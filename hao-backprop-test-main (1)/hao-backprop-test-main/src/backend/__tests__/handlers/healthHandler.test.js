 ...
 * 
 * Tests verify that the handler correctly processes GET requests with JSON
 * { status: 'healthy' } responses and rejects non-GET methods with 
...
describe('handleHealthRequest', () => {
  ...
  it("should return 200 OK with JSON { status: 'healthy' } for GET requests", () => {
    ...
    // Verify Content-Type header was set correctly for JSON
    expect(res.setHeader).toHaveBeenCalledWith(
      HEADERS.CONTENT_TYPE, 
      HEADERS.CONTENT_TYPE_JSON
    );
    
    // Verify response body contains health status JSON
    expect(res.end).toHaveBeenCalledWith(JSON.stringify({ status: MESSAGES.HEALTH_OK }));
    ...
    expect(logger.info).toHaveBeenNthCalledWith(2, `Successfully responded with ${HTTP_STATUS.OK} OK and health status`);
